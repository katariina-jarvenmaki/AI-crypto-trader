...
def execute_bybit_long_limit(symbol, risk_strength, min_inv_diff_percent):
    ...
    print(f"📦 Bybit LIMIT LONG order: {bybit_symbol} {result['qty']} units @ {result['price']} USDT")
    ...
    if order_result:
        print(f"✅ Bybit LIMIT LONG trade placed: TP @ {order_result['tp_price']}, SL @ {order_result['sl_price']}")
        return {
            "symbol": bybit_symbol,
            "direction": "long",
            "qty": qty,
        ...
        print(f"❌ Failed to place Bybit LIMIT LONG trade for symbol {bybit_symbol}")
        return None
